...
static const char* s_shaderRectangleVert = \
    ...
    "void main(void) {"\
    "    gl_Position = uMatrix * vec4(uPosition + aVertices * uPosition.zw, 0.0, 1.0);"\
    "}";
...
void VananaGUI::handleMouseWheel(bool dir) {
    // for (auto e : this->elements)
    //     e->handleMouseWheel(this->mouse, xRel, yRel);
}
